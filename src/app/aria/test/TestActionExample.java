...
/**
 * Example Action class that drives the robot forward, but stops if obstacles
 * are detected by sonar.
 */
class ExampleGoAction extends ArAction {
	...
	/**
	 * This fire is the whole point of the action. currentDesired is the
	 * combined desired action from other actions previously processed by the
	 * action resolver. In this case, we're not interested in that, we will set
	...
	/**
	 * Override ArAction.setRobot() to get the sonar device from the robot, or
	 * deactivate this action if it is missing. You must also call
	 * ArAction.setRobot() to properly store the ArRobot pointer in the ArAction
	 * base class.
	 */
	 ...
	 * the action resolver a pointer to this object. This object is kept as a
	 * class member so that it persists after fire() returns (otherwise fire()
	 * would have to create a new object each invocation, but would never be
	 * able to delete that object).
	 */
...
class ExampleTurnAction extends ArAction {
	...
	public ArActionDesired fire(ArActionDesired currentDesired) {
		...
		// if we're not turning already, but need to, and left is closer, turn
		// right
		// and set the turning variable so we turn the same direction for as
		// long as
		// we need to
		...
		// if we're not turning already, but need to, and right is closer, turn
		// left
		// and set the turning variable so we turn the same direction for as
		// long as
		// we need to
	...
	/**
	 * Calls the ArAction.setRobot() method to store the robot object (all
	 * setRobot overloaded functions must do this), finds the sonar device from
	 * the robot.
	 */
	public void setRobot(ArRobot robot) {
		setActionRobot(robot);
...
public class TestActionExample {
	...
	/*
	 * Try to load the Aria and ArNetworking wrapper libraries when the program
	 * starts:
	 */
	static {
		try {
	...
	public static void main(String argv[]) {
		...
		// with higher priority actions going first, and possibly pre-empting
		// lower
		// priority actions.
