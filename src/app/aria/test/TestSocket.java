...
public class TestSocket {
	...
	public static void main(String argv[]) {
		...
		ArSocket s1client = new ArSocket(); // used by server to communicate
											// with client
		...
		text = s1client.read(512, 5000); // arguments are buffer size and
											// timeout to wait
		/*
		 * while(text.length() == 0) { text = s1client.read(512, 5000); //
		 * arguments are buffer size and timeout to wait }
		 */
